...
export class InMemoryDataService implements InMemoryDbService {
  ...
  createDb() {
    const bookContent: Content[] = [{ // THE BODY IS THE FIRST LINE OF EACH BOOK PLEASE ENJOY LOL
      id: 0,
      author: "Stephen King",
      title: "The Shining",
      type: "Horror",
  ...
  genId(content: Content[]): number{
    return content.length > 0 ? Math.max(...content.map(newBookItem => newBookItem.id ?? 0)) + 1 : 0;
  }
}
